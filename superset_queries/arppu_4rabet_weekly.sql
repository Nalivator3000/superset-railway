-- Упрощенный отчет: ARPPU по бренду 4rabet с группировкой по неделям
...
-- 4. В Chart Builder:
--    - Dimensions: week_start (группировка по неделе)
--    - Metrics: arppu_usd (AVG)
--    - Визуализация: Line Chart или Bar Chart

-- Оптимизированный запрос: один проход по данным, группировка по неделям
SELECT 
    DATE_TRUNC('week', ue.event_date)::date as week_start,
    CASE 
...
GROUP BY DATE_TRUNC('week', ue.event_date)
ORDER BY week_start;
